...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests()//开启登录配置.antMatchers("/login").permitAll()
            ...
            .antMatchers("/hello/callback").permitAll()
            .antMatchers("/hello/haha1").hasRole("admin")//表示访问 /hello 这个接口，需要具备 admin 这个角色
        ...
        /**
         * 配置过滤器
         * 添加JWT过滤器，JWT过滤器要放在UsernamePasswordAuthenticationFilter前，
         * 这时我们将从redis获取的用户信息存入SecurityContextHolder才行，否则后续过滤器在进行校验的时候，
         * 可能会因为SecurityContextHolder中没有对应的值而判断当前访问用户验证不通过。
         */
